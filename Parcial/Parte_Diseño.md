...
#### 2. Captura de Imágenes (Data Acquisition)
...
#### 3. Anotación de Imágenes (Labeling)
...
Estructurar el dataset en carpetas de forma ordenada para facilitar su uso durante el entrenamiento:
dataset_laboratorio/
├── train/
│ ├── osciloscopio/
│ │ ├── img_001.jpg
...
Para identificar múltiples herramientas y personas en una imagen, necesitamos un sistema de **detección de objetos** (que no solo clasifique, sino que también localice cada elemento). MediaPipe Tasks ofrece soluciones listas para esto.

#### 2. Entrenamiento del Modelo (opcional pero recomendado)
...
#### 3. Implementación con MediaPipe Tasks
...
<html>
<head>
    <title>Monitor de Laboratorio</title>
    <style>
        /* Estilos CSS aquí */
        .container { display: flex; }
        .video-container { position: relative; }
        canvas { position: absolute; top: 0; left: 0; }
        .panel { width: 300px; margin-left: 20px; }
    </style>
</head>
<body>
    <h1>Laboratorio de Telecomunicaciones</h1>
    <div class="container">
    ...
    <script>
        // JavaScript para consumir la API y actualizar la vista
        async function fetchData() {
            const response = await fetch('/api/personas');
            const data = await response.json();
            // Dibujar en canvas y actualizar listas
        }
        setInterval(fetchData, 500); // Actualizar cada 0.5 segundos
    </script>
</body>
</html>
...
Detección	MediaPipe, TensorFlow
